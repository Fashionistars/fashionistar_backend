...
from apps.kyc.services import KycGateError
...
class WalletWithdrawalView(generics.GenericAPIView):
    """Create a KYC-gated wallet withdrawal request.

    The service blocks before debit if KYC is incomplete, then moves funds from
    available to pending balance and writes a processing payout ledger row.
    """
    ...
    def post(self, request):
        ...
        try:
            ...
            return success_response(
                data=result,
                message="Withdrawal request created and queued for payout.",
                status=status.HTTP_201_CREATED,
            )
        except ValidationError as exc:
            return error_response(message=str(exc), status=status.HTTP_400_BAD_REQUEST)
        except KycGateError as exc:
...
class EscrowReleaseView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        try:
            result = EscrowService.release_order_payment_for_client(
                client_user=request.user,
