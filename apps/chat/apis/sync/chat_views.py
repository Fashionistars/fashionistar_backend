...
class StartConversationView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        # Fetch the vendor UnifiedUser by the vendor_id from the POST body.
        # The buyer initiates the conversation — the vendor is identified by their user UUID.
        ...
        return success_response(
            ...
            message=(
                ...
                if created
                else "Existing conversation returned."
            ),
...
class MarkConversationReadView(APIView):
    ...
    Mark all messages in a conversation as read for the requesting participant.
    """
    ...
    def post(self, request: Request, conversation_id: str) -> Response:
        ...
        # Verify participant
        if request.user.id not in (conversation.buyer_id, conversation.vendor_id):
        ...
        try:
            count = chat_service.mark_messages_read(conversation, request.user)
        except Exception as exc:
            logger.warning("MarkConversationReadView failed: %s", exc)
            count = 0
