# apps/analytics/workflows/analytics.py
"""
AnalyticsWorkflow — LangGraph state-machine for platform analytics & insights.

This workflow was migrated from apps/ai/workflows/analytics.py to apps/analytics
as part of the analytics engine modernization. AI orchestration (recommendation,
measurement, ingestion) remains in apps/ai.
...
Graph:
  ...
  detect_anomalies
      ↓
  generate_llm_insights      (via apps.analytics.entry_points — optional)
      ↓
  persist_report
      ↓
    END
...
class AnalyticsWorkflow:
    ...
    Aggregates cross-app data via the shared FashionistarDatabaseLayer,
    detects statistical anomalies, and optionally generates LLM-powered
    natural-language insights through apps.analytics.entry_points.
    ...
    def _aggregate_user_metrics(self, state: dict) -> dict:
        ...
        try:
            ...
            # Measurement profile completion rate (read-only analytics use)
            try:
    ...
    def _generate_llm_insights(self, state: dict) -> dict:
        """
        Generate natural-language insights from aggregated metrics via the AI
        entry point. Falls back to empty string if the LLM engine is unavailable.
        """
        from apps.analytics.entry_points import generate_llm_insights
    ...
    def _persist_report(self, state: dict) -> dict:
        ...
        cache_key = f"analytics:report:{state['scope']}:{scope_key}:{state['days']}d"
