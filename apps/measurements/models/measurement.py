...
class MeasurementProfile(TimeStampedModel):
    ...
    owner = models.ForeignKey(
        ...
        related_name="client_measurement_profiles",
    ...
    def has_core_measurements(self) -> bool:
        ...
        """
        return all(v is not None for v in [self.waist, self.hips, self.height]) and (
            self.bust is not None
        )
    ...
    def set_as_default(self):
        ...
        with transaction.atomic():
            # Reverse FK traversal: owner.client_measurement_profiles keeps the
            # default flip scoped to this profile's owner without an unscoped
            # MeasurementProfile table lookup.
            self.owner.client_measurement_profiles.filter(is_default=True).exclude(pk=self.pk).update(is_default=False)
