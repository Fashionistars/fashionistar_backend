...
class VendorProfile(TimeStampedModel, SoftDeleteModel):
    ...
    store_slug = models.SlugField(
        max_length=160, unique=True, blank=True, db_index=True,
    ...
    # ── Analytics (denormalized counters — updated by signals/tasks) ──
    total_products = models.PositiveIntegerField(default=0)
    total_sales    = models.PositiveIntegerField(default=0)
    total_revenue  = models.DecimalField(max_digits=16, decimal_places=2, default=Decimal("0"))
    average_rating = models.DecimalField(max_digits=3, decimal_places=2, default=Decimal("0"))
    review_count   = models.PositiveIntegerField(default=0)

    # ── Wallet ─────────────────────────────────────────────────────
    wallet_balance = models.DecimalField(max_digits=14, decimal_places=2, default=Decimal("0"))
    ...
    def get_average_rating(self) -> float:
        ...
        try:
            return (
                ...
                .aggregate(avg=Avg("review_product__rating"))
                .get("avg") or 0.0
            )
    ...
    def get_wallet_balance(self) -> Decimal:
        ...
        try:
            total = (
                self.vendor_wallet_transactions
    ...
    def get_review_count(self) -> int:
        ...
        """
        try:
            return self.vendor_products.aggregate(
                total=Count("review_product")
            ).get("total") or 0
        except Exception as exc:
    ...
    def get_todays_sales(self) -> Decimal:
        ...
        try:
            ...
            return (
                self.vendor_orders
                .filter(payment_status="paid", date__date=today)
                .aggregate(total=Sum("total"))
    ...
    def get_this_month_sales(self) -> Decimal:
        ...
        try:
            ...
            return (
                self.vendor_orders
                .filter(payment_status="paid", date__month=now.month, date__year=now.year)
                .aggregate(total=Sum("total"))
                .get("total") or Decimal("0")
            )
        except Exception as exc:
    ...
    def get_year_to_date_sales(self) -> Decimal:
        ...
        try:
            ...
            return (
                self.vendor_orders
                .filter(payment_status="paid", date__year=now.year)
                .aggregate(total=Sum("total"))
                .get("total") or Decimal("0")
            )
        except Exception as exc:
    ...
    def calculate_average_order_value(self) -> float:
        ...
        try:
            return (
                self.vendor_orders
                .filter(payment_status="Fulfilled")
    ...
    def get_new_customers_this_month(self) -> int:
        ...
        try:
            ...
            buyers_this_month = (
                self.vendor_orders
                .filter(date__month=now.month, date__year=now.year)
