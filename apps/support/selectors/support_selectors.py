...
def get_ticket_or_none(ticket_id: UUID | str, user) -> Optional["SupportTicket"]:  # noqa: F821
    """
...
def get_admin_ticket_or_none(ticket_id: UUID | str) -> Optional["SupportTicket"]:  # noqa: F821
    """
    Admin-level ticket detail. This is intentionally global and staff-only,
    so it cannot be scoped through a single user's reverse relationship.
    """
...
async def aget_ticket_or_none(
    ...
    except user.submitted_tickets.model.DoesNotExist:
        return None
...
async def aget_ticket_message_thread(ticket) -> list:
    """
...
async def aget_ticket_count(user, *, status: str | None = None) -> int:
    """Async count of user tickets, optionally filtered by status."""
    qs = user.submitted_tickets.all()
    if status:
        qs = qs.filter(status=status)
