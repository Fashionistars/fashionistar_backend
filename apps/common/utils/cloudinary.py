...
def generate_cloudinary_signature(params_to_sign: dict) -> str:
    ...
    sorted_params = "&".join(
        f"{k}={v}" for k, v in sorted(params_to_sign.items()) if v not in ("", None)
    )
...
def generate_cloudinary_upload_params(
    ...
    eager_str = (
        "|".join(
            ",".join(f"{_CLD_KEY_MAP.get(k, k)}_{v}" for k, v in t.items())
            for t in eager
        )
        if eager
        else ""
...
def validate_cloudinary_webhook(
    ...
    *,
    ...
    try:
        ...
        expected = _hs.sha1(
            raw.encode("utf-8")
        ).hexdigest()  # nosec: Cloudinary mandates SHA-1
...
def _sync_upload_one(
    ...
    try:
        ...
        return CloudinaryUploadResult(
            file_path=file_path, error=str(exc), success=False
        )
...
async def async_bulk_delete_media(
    ...
    *,
    ...
    if len(public_ids) > 50:
        ...
        chunks = [
            public_ids[i : i + chunk_size]
            for i in range(0, len(public_ids), chunk_size)
        ]
        sub_results = await asyncio.gather(
            *[asyncio.create_task(_batch_chunk(c)) for c in chunks]
        )
        ...
        results = list(
            await asyncio.gather(
                *[asyncio.create_task(_guarded(pid)) for pid in public_ids]
            )
        )
...
async def async_get_media_info_bulk(
    ...
    *,
    ...
    return list(
        await asyncio.gather(*[asyncio.create_task(_fetch(pid)) for pid in public_ids])
    )
