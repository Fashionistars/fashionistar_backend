...
def vendor_profile(db, django_user_model):
    ...
    profile, _ = VendorProfile.objects.get_or_create(
        user=vendor_user,
        defaults={"store_name": "Test Cart Shop", "is_verified": True},
    )
    return profile
...
class TestCartService:
    ...
    def test_merge_anonymous_cart_session_is_idempotent(self, client_user, product):
        """
        Database-backed anonymous cart rows promote to the user cart once.

        A second merge with the same session_key must be a no-op so login and
        checkout can safely retry the reconciliation call.
        """
        session_key = "guest-cart-session-123"
        add_item(session_key=session_key, product_slug=product.slug, quantity=2)

        cart = merge_anonymous_cart_session(user=client_user, session_key=session_key)
        assert Cart.objects.filter(session_key=session_key, user__isnull=True).count() == 0
        assert cart.items.count() == 1
...
class TestCartAPI:
    ...
    def test_anonymous_gets_empty_cart(self, api_client):
        """
        Ninja cart endpoint returns 200 with empty cart for anonymous users.
        This supports the anonymous commerce contract (session_key-based carts).
        DRF write endpoints (add/remove/clear) remain gated at 401.
        """
