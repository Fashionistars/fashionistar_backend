...
class UnifiedUserBackend(BaseBackend):
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        try:
            ...
            user = UnifiedUser.objects.filter(
                Q(email=username, is_deleted=False) if "@" in username else Q(phone=username, is_deleted=False)
            ).first()
    ...
    async def aauthenticate(self, request, username=None, password=None, **kwargs):
        ...
        try:
            try:
              # ✅ OPTIMIZED: Single database query using Q object (async)
                user = await UnifiedUser.objects.filter(
                    Q(email=username, is_deleted=False) if "@" in username else Q(phone=username, is_deleted=False)
                ).afirst()
            except UnifiedUser.DoesNotExist:
                logger.warning(f"User with identifier '{username}' not found during async authentication.")
                user = None
            except Exception as e:
                logger.error(f"Error querying UnifiedUser (async): {str(e)}")
                user = None
