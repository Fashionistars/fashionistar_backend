    ...
    def register_sync(
        ...
        **extra_fields
        ...
        try:
            ...
            # ── Audit trail: USER_REGISTERED (deferred inside on_commit) ─────
            _user_id_str = str(user.id)
            _user_email = str(user.email) if user.email else None
            _user_phone = str(user.phone) if user.phone else None
            _user_role = str(user.role) if user.role else role
            ...
            transaction.on_commit(_audit_registered)
