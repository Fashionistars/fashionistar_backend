...
class ResendOTPRequestSerializer(serializers.Serializer):
    ...
    email_or_phone = serializers.CharField(
        required=True, help_text="Registered email or phone number."
    )
    ...
    def validate(self, data):
        ...
        try:
            ...
            if user is None:
                ...
                if (
                    UnifiedUser.objects.all_with_deleted()
                    .filter(
                        (
                            Q(email=email_or_phone)
                            if "@" in email_or_phone
            ...
            logger.error(
                "❌ Unexpected error in LoginSerializer: %s", exc, exc_info=True
            )
            raise serializers.ValidationError(
...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            if (email or phone) and UnifiedUser.objects.all_with_deleted().filter(
                (Q(email__iexact=email) if email else Q(phone=phone)), is_deleted=True
            ).exists():
                logger.warning(
                    "⛔ Registration blocked: soft-deleted account '%s' or '%s'",
                    email,
                    phone,
                )
                ...
                raise serializers.ValidationError(
                    {
                        "email" if email else "phone": [
                            _(
                                ...
                                if email
                                else "A user with this phone number already exists."
                            )
                        ]
                    }
                )
            ...
            logger.error(
                "Unexpected error in registration validation: %s", exc, exc_info=True
            )
            raise serializers.ValidationError(
    ...
    def create(self, validated_data):
        try:
            ...
            logger.error(
                "❌ Error creating user via serializer: %s", exc, exc_info=True
            )
            raise serializers.ValidationError(
    ...
    def validate(self, attrs):
        try:
            ...
            raise serializers.ValidationError(
                {
                    "non_field_errors": _(
                        "An error occurred during Google Auth validation."
                    )
                }
            )
