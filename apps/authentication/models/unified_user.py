...
class UnifiedUser(HardDeleteMixin, AbstractUser, TimeStampedModel, SoftDeleteModel):
    ...
    def clientprofile(self):
        """
        Backward-compatible reverse accessor alias for client profile.

        The canonical related_name is ``client_profile``. This alias supports
        older service code and the shorter `request.user.clientprofile` pattern
        without adding another database relationship.
        """
        try:
    ...
    def vendorprofile(self):
        """
        Backward-compatible reverse accessor alias for vendor profile.

        The canonical related_name is ``vendor_profile``. This alias supports
        older service code and the shorter `request.user.vendorprofile` pattern
...
try:
    ...
    pass  # auditlog not installed or already registered (test reloads)
