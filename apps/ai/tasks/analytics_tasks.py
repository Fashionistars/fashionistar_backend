# apps/ai/tasks/analytics_tasks.py
"""
Celery tasks for the FASHIONISTAR AI Analytics pipeline.
...
Queue: "ai" (dedicated queue for ML-heavy + analytics tasks)
...
@shared_task(
    bind=True,
    name="apps.ai.tasks.analytics_tasks.run_platform_analytics",
    queue="ai",
    max_retries=2,
...
def run_platform_analytics(
    ...
    """
    Full platform analytics pipeline via AnalyticsWorkflow.
    ...
    Detects anomalies + generates LLM insights (if Ollama available).
    ...
    try:
        from apps.ai.workflows.analytics import AnalyticsWorkflow
...
@shared_task(
    bind=True,
    name="apps.ai.tasks.analytics_tasks.run_user_behavior_analysis",
    queue="ai",
    max_retries=2,
    soft_time_limit=120,
    time_limit=150,
...
def run_user_behavior_analysis(self, user_id: int, days: int = 30) -> dict:
    ...
    Cached in Redis at key: ai:analytics:user:{user_id}
    ...
    try:
        ...
        cache_key = f"ai:analytics:user:{user_id}"
...
@shared_task(
    bind=True,
    name="apps.ai.tasks.analytics_tasks.run_product_performance_analysis",
    queue="ai",
    max_retries=2,
    soft_time_limit=120,
    time_limit=150,
...
def run_product_performance_analysis(self, product_id: int, days: int = 30) -> dict:
    ...
    try:
        ...
        cache_key = f"ai:analytics:product:{product_id}"
...
@shared_task(
    name="apps.ai.tasks.analytics_tasks.generate_daily_report",
    queue="ai",
