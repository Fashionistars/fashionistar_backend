...
class TestRegisterEndpoint(TestCase):
    ...
    OTPService.generate_otp_sync and email dispatch are mocked because
    the test environment has no Redis or Celery broker.  The mocks isolate
    the HTTP / serializer / DB layer which is what these tests verify.
    """
    ...
    def setUp(self):
        ...
        # ── Mock OTP generation (Redis not available in test env) ──────────
        self._otp_patch = patch(
            "apps.authentication.services.otp.sync_service.OTPService.generate_otp_sync",
            return_value="123456",
        )
        self._otp_patch.start()

        # ── Mock email task (Celery broker not available in test env) ──────
    ...
    @skipIf(
        "sqlite" in settings.DATABASES.get("default", {}).get("ENGINE", ""),
        "SQLite cannot handle concurrent multi-thread writes; run against PostgreSQL.",
    )
    def test_concurrent_registrations_different_emails_all_succeed(self):
        """20 concurrent registrations for different emails — all must succeed.

        Requires PostgreSQL.  SQLite serialises writers and raises
        'database table is locked' under concurrent thread load, which makes
        this test a false failure in the local dev environment.
        """
