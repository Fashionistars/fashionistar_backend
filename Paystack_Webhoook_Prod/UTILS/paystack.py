...
import secrets
import requests
from backend.config.base import PAYSTACK_TEST_KEY, PAYSTACK_SECRET_KEY
        ...
        try:
            ...
            if "res" in locals():
                if res.status_code == 500:
                    ...
                    print(
                        f"Error from paystack API: {e}, response: {res.text if 'res' in locals() else None}"
                    )
                    return {
                        "status": False,
...
class Refund:
    ...
    def __init__(self, reference):
        self.body = {"transaction": reference}
